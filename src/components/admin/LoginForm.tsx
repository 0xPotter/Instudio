"use client";

import Image from "next/image";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { useAuth } from "@/lib/firebase/AuthProvider";
import { signInWithGoogle } from "@/lib/firebase/auth";

export function LoginForm() {
  const router = useRouter();
  const { user, loading } = useAuth();
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Already authenticated → go to admin dashboard.
  useEffect(() => {
    if (!loading && user) router.replace("/admin");
  }, [loading, user, router]);

  async function handleSignIn() {
    setSubmitting(true);
    setError(null);
    try {
      await signInWithGoogle();
      router.replace("/admin");
    } catch (err) {
      if (err instanceof Error && err.message === "unauthorized") {
        setError("Access denied. Your account is not authorized.");
      } else {
        setError("Sign-in failed. Please try again.");
      }
    } finally {
      setSubmitting(false);
    }
  }

  if (loading || user) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-neutral-950">
        <span className="font-label text-[10px] uppercase tracking-[0.3em] text-neutral-500 animate-pulse">
          Loading…
        </span>
      </div>
    );
  }

  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-950 px-6 py-12">
      <div className="w-full max-w-md">
        {/* Logo + heading */}
        <div className="mb-12 flex flex-col items-center gap-6">
          <Image
            src="/logo.svg"
            alt="IN"
            width={48}
            height={58}
            priority
            className="h-12 w-auto"
          />
          <div className="flex flex-col items-center gap-2">
            <h1 className="font-headline text-2xl font-black uppercase tracking-tight text-white">
              IN Studio
            </h1>
            <span className="font-label text-[10px] uppercase tracking-[0.3em] text-neutral-500">
              Administrative Control
            </span>
          </div>
        </div>

        {/* Card */}
        <div className="border border-neutral-900 bg-neutral-950 p-10">
          <div className="flex flex-col gap-8">
            <p className="text-center font-label text-[10px] uppercase tracking-widest text-neutral-500">
              Identity / Authentication
            </p>

            {error && (
              <p className="text-center font-label text-[10px] uppercase tracking-widest text-red-400">
                {error}
              </p>
            )}

            <button
              type="button"
              onClick={handleSignIn}
              disabled={submitting}
              className="mt-2 flex w-full items-center justify-center gap-3 bg-white py-4 font-label text-[11px] uppercase tracking-[0.25em] text-black transition-all duration-200 hover:bg-neutral-200 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <svg className="h-4 w-4" viewBox="0 0 24 24">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              {submitting ? "Initializing…" : "Sign in with Google"}
            </button>
          </div>
        </div>

        {/* Status footer */}
        <div className="mt-10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-emerald-400" />
            </span>
            <span className="font-label text-[9px] uppercase tracking-[0.25em] text-neutral-500">
              System Online
            </span>
          </div>
          <span className="font-label text-[9px] uppercase tracking-[0.25em] text-neutral-600">
            Terminal v2.4.0
          </span>
        </div>
      </div>
    </div>
  );
}
